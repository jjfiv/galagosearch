 ...
 *
 */
public class MemoryRetrieval extends Retrieval {
  ...
  public MemoryRetrieval(MemoryIndex index, Parameters factoryParameters) throws IOException {
    ...
    MemoryParameters featureParameters = new MemoryParameters(this, factoryParameters);

    featureParameters.add("retrievalGroup", "all"); // the value wont matter
    // here
  ...
  /*
   * <parameters> <collectionLength>cl<collectionLength>
   * <documentCount>dc<documentCount> </parameters>
   */
  ...
  /*
   * <parameters> <part> (partName)+ </part> <nodeType>
   * <(partName)_(nodeType)>(class)</(partName)_(nodeType)> </nodeType>
   * </parameters>
   */
   ...
   *
   * @param query
   *            A query tree that has been already transformed with
   *            StructuredRetrieval.transformQuery.
   * @param parameters
   *            - query parameters (indexId, # requested, query type,
   *            transform)
   * @return
  ...
  /**
   * Evaluates a query using document-at-a-time evaluation. - allowing user to
   * sweep accross parameters specified within the query
   *
   * @param query
   *            A query tree that has been already transformed with
   *            StructuredRetrieval.transformQuery.
  ...
  /**
   * Returns the number of occurrences of the provided expression. If the
   * expression does not produce a CountIterator as a node type, throws an
   * IllegalArgumentException, since it's not an appropriate input. #text,
   * #ow, and #uw should definitely be ok here.
   *
   * @param nodeString
  ...
  public long docCount(Node root) throws Exception {
    ...
    if (structIterator instanceof CountIterator) {
      CountIterator iterator = (CountIterator) structIterator;
      long count = 0;
      do { // works for iterators that are always done (#all - see
        // ExtentIndexReader)
        count += 1;
        iterator.nextEntry();
      } while (!iterator.isDone());
