...
@OutputClass(className = "org.galagosearch.core.types.Adjacency")
public class DistanceCalculator extends StandardStep<KeyValuePair, Adjacency> {
  ...
  public DistanceCalculator(TupleFlowParameters parameters) throws Exception {
    ...
    StructuredIndexPartReader partReader = new PositionIndexReader(StructuredIndex.getPartPath(indexLocation, partName));
    counter = parameters.getCounter("pairs calculated");
    iterator = partReader.getIterator();
    String method = parameters.getXML().get("method", "levenshtein");
      ...
      while (iterator.nextKey()) {
        ...
        try {
          ...
          if (result <= maxdistance) {
            a = new Adjacency();
            a.source = object.key;
            a.destination = Utility.fromInt(index);
            a.weight = result;
            processor.process(a);
            if (counter != null) {
              counter.increment();
