...
public class BuildFastIndex {
  ...
  public Stage getParsePostingsStage() {
    ...
    if(makeCorpus){
      ...
      corpusParams.add("filename", corpusPath);
      corpus.add(new Step(CorpusDocumentWriter.class, corpusParams));
      corpus.add(Utility.getSorter(new CorpusIndexItem.IdentifierOrder()));
      corpus.add(new OutputStep("corpusIndexData"));
  ...
  public Stage getCorpusWriter() {
    Stage stage = new Stage("corpusIndexWriter");
    
    stage.add(new StageConnectionPoint(
        ConnectionPointType.Input, "corpusIndexData", 
        new CorpusIndexItem.IdentifierOrder()));
    
    stage.add(new InputStep("corpusIndexData"));
    Parameters p = new Parameters();
  ...
  public Job getIndexJob(Parameters p) throws IOException{
    ...
    this.makeCorpus = (corpusPath.length() > 0);
